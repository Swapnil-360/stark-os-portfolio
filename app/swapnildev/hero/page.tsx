...
export default function AdminHeroPage() {
  ...
  const handleSaveAll = async () => {
    setIsSaving(true);
    setSaveMessage("Saving all parameters to Supabase...");
    try {
      await updateHero(formData);
      setSaveMessage("Saved & published live to all visitors!");
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={isSaving}
            className="px-4 py-2 bg-red-600 hover:bg-red-500 disabled:bg-slate-800 text-white rounded-lg text-xs font-mono font-bold flex items-center gap-2 transition shadow-lg shadow-red-950/40 cursor-pointer disabled:cursor-not-allowed"
          >
            {isSaving ? (
              <>
          ...
          <div className="px-3.5 py-2 bg-emerald-950/40 border border-emerald-800/80 text-emerald-400 rounded-full text-xs font-mono flex items-center gap-2 shadow-sm">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span>{saveMessage || (isSavedRecently ? "Synced Just Now" : "Auto-Save Active // Instant Sync")}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {/* Bottom Save Bar */}
          <div className="p-5 bg-[#11141c] border border-slate-800 rounded-lg flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
            <div className="flex items-center gap-2 text-xs font-mono text-slate-300">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span>{saveMessage || (isSavedRecently ? "All modifications saved live to Supabase." : "Ensure all changes are saved and synced.")}</span>
            </div>
            <button
              type="button"
              onClick={handleSaveAll}
              disabled={isSaving}
              className="w-full sm:w-auto px-6 py-2.5 bg-red-600 hover:bg-red-500 disabled:bg-slate-800 text-white rounded-lg text-xs font-mono font-bold flex items-center justify-center gap-2 transition shadow-lg cursor-pointer disabled:cursor-not-allowed"
            >
              {isSaving ? "Saving to Supabase..." : "Save All Hero Settings"}
            </button>
          </div>
        </div>
